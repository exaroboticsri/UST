        ...
        try:
            if pipeChild.poll(None):
                ...
                if cmd == "CREATE":
                    ...
                    on_callback(gROSNode.callback ,feedbackQueueBk)
                    # rclpy.spin(gROSNode)
                    ...
                    print('mp Face TARGET', str(val))
                    gROSNode.set_target_name(str(val))
                    
                elif cmd == "FOLLOW_MODE" :
                    print('mp Face FOLLOW_MODE', bool(val))
                    gROSNode.set_follow_mode(bool(val))
                
                elif cmd == "FRAME":
                    # print('mp Face FRAME')
                    gROSNode.callback(val)
...
def on_pub_cam_frames(cam_frames, q: Queue):
    # print('get cam_frames ')
    q.put(cam_frames)



def on_pub_pos_xyz(pos_data: Tuple[float, float, float, float, float, float, float], q: Queue):
    print('recv pos_xyz')
    dict_msg = dict()
...
def on_target_name(name:str):
    print('mp Face TARGET', str(name))
    gROSNode.set_target_name(str(name))

def on_follow_mode(mode:bool):                
    print('mp Face FOLLOW_MODE', bool(mode))
    gROSNode.set_follow_mode(bool(mode))

def on_callback(fun, q: Queue):
    dict_msg = dict()
    dict_msg['CALLBACK'] = fun
    q.put(dict_msg)