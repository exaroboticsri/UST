...
from Commons import convert_s16_to_bytearray_be, convert_bytearray_to_int_be
...
class CExternalCommClient:
    ...
    def deserialize_packet(self):
        ...
        if len(self.btaRecvBuff) >= n_next_packet_idx:
            self.btaSerialBuff = self.btaRecvBuff[n_next_packet_idx:]
    ...
    def parse_packet_values(self, abtData):
        len = int(abtData[2])
        b_cmd = int(abtData[3])
        if  b_cmd == EnumExternalCommCmd.GET_TTS_CMD:
            # msg= float(convert_bytearray_to_int_be(abtData[4:6], abIsSigned=True) / 1000.)
            msg = bytes(abtData[4:len+3]).decode()
            self.sig_tts_msg.emit(msg)
