...
class CExternalComm:
    ...
    def deserialize_packet(self):
        ...
        # print('deserialize_packet 1 ',self.btaRecvBuff)
        ...
        if n_idx is not None:
            ...
            # print('deserialize_packet 2 ',self.btaRecvBuff)
            if len(self.btaRecvBuff) >= n_idx + 5:
                # print('deserialize_packet 3 ',self.btaRecvBuff)
                n_packet_len = self.btaRecvBuff[n_idx + 2] + 4  # only data and CMD are counted in packet length
                if len(self.btaRecvBuff) >= n_idx + n_packet_len:
                    # print('deserialize_packet 4 ',self.btaRecvBuff)
                    if self.btaRecvBuff[n_idx + n_packet_len - 1] == 0x5D and self.btaRecvBuff[
                        n_idx + n_packet_len] == 0x03:
                        # print('deserialize_packet 5 ',self.btaRecvBuff)
    ...
    def parse_packet_values(self, abtData):
        ...
        # print('parse_packet_values: ',abtData, b_cmd)
        ...
        if b_cmd == EnumExternalCommCmd.EMG_STOP:
            ...
            # print('data: ',x_pos, y_pos, aAngle)
            self.sig_move_pose.emit(x_pos, y_pos, aAngle)
            # print(abtData, 'get MOVE_POSE data !!',
            #       abtData[4:6], abtData[6:8], x_pos, y_pos)
    ...
    ####
    def send_tts_msg(self, msg: str):
        byte = msg.encode()
        payload = bytearray(byte)
        self.send_packet(EnumExternalCommCmd.GET_TTS_CMD, payload)
