...
class DockProcessMonitor(QtWidgets.QDockWidget):
    ...
    def initUI(self):
        ...
        self.spinAutoRefresh.setMinimum(100)
    ...
    def on_change_state(self, state: int):
        if state == QtCore.Qt.Checked:
            ...
            if self._threadUpdateTable is not None:
                self._threadUpdateTable.set_cpu_get_period(period)
...
class ThreadUpdateTable(QtCore.QThread):
    ...
    def set_cpu_get_period(self, anInMs: int):
        self._period = float(anInMs/1000.)
    ...
    def run(self) -> None:
        while self._keepAlive:
            try:
                dict_proc = self._queue.get(block=True)
                dict_update = dict()
                ...
                for proc_name in list_proc_name:
                    ...
                    if pid_no is not None:
                        ...
                        cpu_usage = this_proc.cpu_percent(self._period)
                ...
                self.write_log_test(dict_update)
    ...
    def write_log_test(self, obj,logfile: bool = True):
        global glob_logger2
        if glob_logger2 is None:
            curpath = os.path.dirname(os.path.abspath(__file__))
            dirpath = os.path.dirname(curpath)
            _dirpath = os.path.dirname(dirpath)
            logpath = os.path.join(_dirpath, 'Log')
            if not os.path.isdir(logpath):
                os.mkdir(logpath)
            logfilepath = os.path.join(logpath, 'load.log')
            glob_logger2 = logging.getLogger('load')
            fh = logging.handlers.RotatingFileHandler(logfilepath, maxBytes=MAX_SIZE, backupCount=10, encoding='utf-8')
            formatter = logging.Formatter('[%(asctime)s]%(message)s')
            fh.setFormatter(formatter)
            glob_logger2.addHandler(fh)
            glob_logger2.setLevel(logging.DEBUG)

        if obj is not None:
            list_proc_name = obj.keys()
            dat = []
            for proc_name in obj.keys():
                info = obj[proc_name]
                # print(proc_name, info)
                
                result = '%s %s'% (proc_name, info["CPU_USAGE"])
                dat.append(result)
                # print(result, dat)
